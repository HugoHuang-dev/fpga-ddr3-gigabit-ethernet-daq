...
// File    : udp_v5_tx_fifo_packetizer.v
// Module  : udp_v5_tx_fifo_packetizer
// Created : 2026-08-11
// Revised : 2026-09-22
...
// Two-packet TX FIFO plus UDP packetizer.  A DDR slot is eligible for release
// only when user_rd_last is accepted into this memory.
module udp_v5_tx_fifo_packetizer #(
    parameter integer DATA_BYTES = 1024,
    parameter integer FIFO_PACKETS = 2,
    ...
    output reg  [11:0] fifo_byte_count,
    output reg  [1:0]  fifo_packet_count,
    ...
    always @(posedge clk) begin
        if (reset) begin
            ...
            if (user_rd_valid && fifo_byte_count == FIFO_BYTES)
                framing_error <= 1'b1;
