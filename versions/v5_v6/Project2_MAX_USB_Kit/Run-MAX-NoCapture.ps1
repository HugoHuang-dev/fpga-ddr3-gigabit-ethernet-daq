...
if (-not $principal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
    throw 'Right-click 01_Run_MAX_60s_NoCapture.cmd and choose Run as administrator.'
}
...
$rule = 'Project2_MAX_' + [guid]::NewGuid().ToString('N')
$ruleAdded = $false
$receiverExitCode = -1
...
'@ -Name NativeMethods -Namespace Project2

Start-Transcript -Path (Join-Path $runDir 'console.txt') | Out-Null
try {
    [void][Project2.NativeMethods]::SetThreadExecutionState([uint32]2147483649)
    $sleepGuardActive = $true
    Write-Host "RESULTS: $runDir"
    Write-Host "This is the controlled MAX unpaced NO-CAPTURE test for $DurationSeconds seconds."
    ...
    Write-Host 'Confirm PktMon is stopped. Program fpga\project2_v5_max_unpaced.bit with the matching LTX.'
    ...
    if ($sleepGuardActive) {
        [void][Project2.NativeMethods]::SetThreadExecutionState([uint32]2147483648)
    }
    if ($ruleAdded) {
