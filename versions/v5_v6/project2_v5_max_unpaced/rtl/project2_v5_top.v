...
module project2_v5_top (
    ...
    prbs16_burst_source #(.BURST_BYTES(BURST_BYTES),.PRBS_SEED(16'hACE1)) u_source(
        .clk(clk_125m),.reset(reset),.enable(start_latched),
    ...
    // Unpaced architectural-limit characterization.
    // No additional post-ready idle cycles are inserted; the inherited UDP
    // ready handshake and the packet data path alone determine throughput.
    udp_v5_tx_fifo_packetizer #(.DATA_BYTES(BURST_BYTES),.FIFO_PACKETS(2),
        .POST_READY_IDLE_CYCLES(0)) u_packetizer(
