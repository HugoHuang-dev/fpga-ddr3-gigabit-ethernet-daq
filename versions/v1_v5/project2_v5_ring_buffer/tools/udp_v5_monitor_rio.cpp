...
constexpr int kMonitorVersion = 6;
...
struct Options {
    ...
    int rcvbuf_mb = 64;
    std::string output = "evidence\\v5_60s_ipv4_checksum_fix_result.json";
};
...
int main(int argc, char** argv) {
    ...
    if (!parse_args(argc, argv, options)) {
        std::cerr << "Usage: udp_v5_monitor_rio.exe [--fpga-ip IP] [--pc-ip IP] "
    ...
    std::cout << "Project2 v5 Windows RIO PC monitor v" << kMonitorVersion << "\n";
    ...
    auto process_packet = [&](const uint8_t* buffer, ULONG length) {
        if (length != kPacketBytes || std::memcmp(buffer, "P2V5", 4) != 0 ||
        ...
        if (!payload_matches(buffer + kHeaderBytes, sequence, period, packet_bad_bytes)) {
            ++data_error_packets;
