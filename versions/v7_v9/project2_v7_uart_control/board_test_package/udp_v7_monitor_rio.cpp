...
constexpr int kMonitorVersion = 7;
...
struct Options {
    ...
    int rcvbuf_mb = 64;
    std::string output = "evidence\\v7_uart_control_result.json";
};
...
int main(int argc, char** argv) {
    ...
    if (!parse_args(argc, argv, options)) {
        std::cerr << "Usage: udp_v7_monitor_rio.exe [--fpga-ip IP] [--pc-ip IP] "
    ...
    std::cout << "Project2 v7 Windows RIO PC monitor v" << kMonitorVersion << "\n";
    ...
    auto process_packet = [&](const uint8_t* buffer, ULONG length) {
        ...
        const bool payload_size_valid = payload_size == 256 || payload_size == 512 ||
                                        payload_size == 1024;
        if (length < kHeaderBytes || std::memcmp(buffer, "P2V7", 4) != 0 ||
            buffer[4] != 7 || buffer[5] != 0 || be16(&buffer[6]) != length ||
    ...
    while (true) {
        ...
        if (!started && armed_seconds >= options.first_timeout) {
            std::cerr << "FAIL: no v7 packet received before first-packet timeout.\n";
            break;
        }
    ...
    std::cout << (passed ? "V7 UART-CONTROLLED STREAM TEST PASSED\n"
                        : "V7 UART-CONTROLLED STREAM TEST FAILED\n");
    rio.RIODeregisterBuffer(rio_buffer_id);
    rio.RIOCloseCompletionQueue(completion_queue);
