...
/*------------------------------------------*\
                assign 
\*------------------------------------------*/
assign rd_req_ready = reset_sync ? 0 : cmd_wrcount < 'd12; //留一点余量
...
/*--------------------------------------------------*\
	将rd_ctrl模块传来的地址和突发长度写入cmd fifo
\*--------------------------------------------------*/
always @(posedge clk) begin
    if (rd_req_en) begin
    	cmd_wren <= 1'b1;
...
generate
	if (AXI_DATA_WIDTH == 256) begin
		...
		assign rd_data_buf_ready = a_reset_sync ? 0 :data_wrcount <= 'd384;
		...
		assign rd_data_buf_ready = a_reset_sync ? 0 :data_wrcount <= 'd256;
