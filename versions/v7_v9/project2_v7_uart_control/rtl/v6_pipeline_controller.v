...
module v6_pipeline_controller #(
    ...
    always @(posedge clk) begin
        if(reset) begin
            ...
            if(!start_sync && occupied_slots!=0) drain_active<=1;
            else if(occupied_slots>=HIGH_SLOTS) drain_active<=1;
            ...
            // The UART controller only issues this while the complete pipeline
            // is stopped and empty, so clearing diagnostic state cannot hide an
            // in-flight transaction.
            if(clear_counters) begin
                committed_bytes<=0; released_bytes<=0;
                write_stall_cycles<=0; read_stall_cycles<=0;
                ring_overflow_count<=0; ring_underflow_count<=0;
                fatal_error<=0;
            end
        end
    end
