...
The complete board workflow is in [`BOARD_TEST_V8.md`](../project2_v8_xadc_acquisition/BOARD_TEST_V8.md). It explicitly uses two
...
[`evidence/V8_BOARD_VALIDATION_EVIDENCE_20260922.md`](../project2_v8_xadc_acquisition/evidence/V8_BOARD_VALIDATION_EVIDENCE_20260922.md).
...
## Thirty-first experiment: V9 full-validation instrumentation and local gates

V9 was created as a separate output tree on 2026-09-23. It retains the V8
physical sources (internal four-channel XADC and deterministic PRBS16) and the
ingress FIFO → DDR3 ring → UDP path; there is still no external AD9226 board.
The V9 packetizer adds a 32-byte P2V9 header with a 64-bit first-word index
alongside the 32-bit packet sequence. The corresponding UART status identifies
version 9. Three independent ILA cores observe the 125 MHz acquisition/control
domain, MIG ui_clk DDR/TX domain, and PHY RX domain; no foreign-domain bus is
directly probed by another clock's ILA.

The Windows RIO V9 receiver checks packet and sample origins and continuity
independently. For source 0 it reconstructs PRBS16 from each explicit sample
index and compares every payload byte online. For source 1 it checks the
record's reserved bits, channel order, count, min/max and sample index; unknown
analog measurements are not falsely compared against a predetermined voltage.
It reports live and final throughput plus bounded error counters, writes only
JSON summaries and bounded UDP prefix fragments, and does not dump the full
long-run stream. Its ten self-tests passed, including separate injected packet
gap, sample-index gap, payload corruption, header and XADC format/order faults.

ModelSim DE-64 10.6c compiled and ran three self-checking V9 tests. The ingress
FIFO test covered ordering, watermarks, backpressure, flush and overflow/
underflow injection. The packetizer test covered exact P2V9 framing, bytewise
payload, sequence/index advancement, packet-boundary backpressure and
early/late-last/fifo faults. The pipeline-controller test covered ring pointer
wrap, committed/released occupancy, drain hysteresis, AXI backpressure and
fatal/error injection. All three tests passed; transcripts are retained.

Vivado 2018.3 completed synthesis, placement, routing, DRC and bitstream for
xc7a35tfgg484-2. The fully routed timing report gives WNS +0.292 ns,
TNS 0.000 ns, WHS +0.014 ns and THS 0.000 ns. Routed utilization is 16,976
Slice LUTs (81.62%), 21,459 Slice Registers (51.58%), 26/50 Block RAM Tiles
(52%), 0 DSP and 1 XADC. DRC has 0 errors, 37 warnings and 2 advisories.
The separate bus-skew report has 19 constrained rows, with minimum slack
+6.870 ns. The BIT and debug-probe LTX were generated successfully. The V9
board package and pre-board evidence record contain exact artifact hashes.

This is a **local/pre-board PASS only**. V9 board gates remain open: source 0
60-second quick regression; source 1 finite 32,768 records; source 1 300-second
real-acquisition stream; three separate on-board ILA captures; and source 0
3,600-second online-comparison endurance run. V8 board results cannot close
these V9 gates. Their commands, expected counters and bounded evidence rules
are in BOARD_TEST_V9.md. No V9 board PASS is claimed at this stage.
At handoff this PC exposed only Bluetooth serial ports, not COM4, and the FPGA
address 192.168.1.11 did not answer a ping. Board programming/testing was
therefore not attempted; it requires the board to be connected and powered.

## Thirty-second experiment: V9 board Gates 1–3 and three-domain ILA evidence

On 2026-09-23 the connected board was operated over COM14. The V9 receiver
results and ILA captures were archived. The canonical originals under `board_test_package/results`
were read, and stable copies of the completed JSON, bounded sample fragments,
and four native ILA exports were archived under `evidence/board_20260923`.
Each `.ila` contains `waveform.csv`; the CSV was also extracted into the archive
for direct inspection. The evidence manifest records SHA-256 hashes and the
status of the board-test screenshots.

Gate 1, the completed manual source-0 PRBS16 run, passed for 60.000168 s:
2,913,988 packets, 2,983,923,712 payload bytes, and 397.855381 Mb/s average
UDP payload rate. First packet sequence and first word index were zero. Packet
loss/reordering, sample-index faults, payload-byte faults, metadata faults,
malformed packets, and receive-completion faults were all zero. The separate
earlier [`v9_gate1_prbs_1024B_25M_60s.json`](board_test_package/results/v9_gate1_prbs_1024B_25M_60s.json) contains `passed=false`, duration
zero and zero packets; it is retained as an incomplete attempt, not counted
as the successful Gate 1 run or silently discarded.

Gate 2 source-1 finite acquisition passed: exactly 32,768 XADC records,
64 packets, 65,536 payload bytes, and 8,192 records per channel. First packet
sequence and first word index were zero, and all PC integrity/error fields
were zero. The supplied post-run UART status shows `finite_done=True`,
`run_words=32768`, `packet_sequence=64`, no fatal state, zero ingress and DDR
occupancy, `xadc_valid_mask=0xF`, `xadc_drop_count=0`, and zero listed internal
overflow/underflow counters.

Gate 3 source-1 continuous acquisition passed for 300.0674324 s: 624 packets,
638,976 payload bytes, 319,488 XADC records, exactly 79,872 per channel, and
0.0170355 Mb/s average payload rate. Sequence and first-word origins were
zero; all PC sequence, sample-index, format/order, payload, metadata and
receive errors were zero. Post-STOP status shows an empty ingress and DDR
ring, no fatal state, XADC mask `0xF` and drop count zero. It also reports
`tx_underflow=25`. This counter is **not** represented as zero; the slow-source
idle/starvation interpretation is the previously documented V8 exception,
supported here by intact PC continuity, zero XADC drop and empty post-STOP
pipeline. It is not evidence of a truncated or missing packet.

All four archived ILA CSVs have 1,024 samples with a real trigger at sample
512. The acquisition-domain capture has source select `0x01`, `source_valid=1`,
`source_ready=1`, mask `0xF` and XADC drop zero at the trigger. The PHY RX
capture has `gmii_rx_data_vld=1` and `gmii_rx_data_error=0`. The MIG/UI commit
capture has `tx_burst_committed=1` and `fatal_error=0`. The fourth capture has
`packet_done=1`, `fatal_error=0`, with packet sequence 0→1 and first-word
index 0→512. The packet-done capture was made separately after the 300-second
Gate 3 run; it proves that local event but is **not** claimed as synchronized
with the Gate 3 PC JSON.

The 20 board-test images were reviewed and mapped to Gate 1/2/3, ILA and
Gate 5 setup. Stable copies of all 20 original-named PNGs are stored in
`evidence/board_20260923/screenshots`.
All 20 filenames match the evidence index with no extra/missing files, PNG
signatures are valid, and SHA-256 values were added to the archive manifest.
The Gate 5 configuration image is only a setup record: its 3,600-second run
was still in progress, and no final Gate 5 JSON, PASS image or post-STOP
status was available at this audit.

**V9 current status:** ModelSim, PC self-tests, routed Vivado implementation,
board Gates 1–3, four ILA waveform exports and the 20 provided PNG screenshots
are evidenced. Gate 5 final long-run acceptance remains open. Do not mark V9
complete until its final JSON/PASS and post-STOP status are audited.

The paragraph above is the status at the time of Experiment 32; Experiment
33 below supersedes it with the audited final Gate 5 result.

## Thirty-third experiment: V9 one-hour endurance and final measured disposition

On 2026-09-23 the Gate 5 source-0 PRBS16, 1,024-byte, 25 Mword/s endurance
run completed. The original
[`board_test_package/results/v9_gate5_prbs_1024B_25M_3600s.json`](board_test_package/results/v9_gate5_prbs_1024B_25M_3600s.json) and the two
final/status screenshots were inspected and copied, together
with all 60 bounded 256-byte sample fragments, into
`evidence/board_20260923`. The additional 63 files are listed and verified
in [`GATE5_SHA256.txt`](evidence/board_20260923/GATE5_SHA256.txt); the prior 44-file manifest remains intact. The final
evidence index is [`V9_BOARD_EVIDENCE_FINAL.md`](evidence/board_20260923/V9_BOARD_EVIDENCE_FINAL.md) in that archive directory.

The run lasted 3600.0000886 seconds and received 174,834,426 packets, or
179,030,452,224 payload bytes, at 397.845440 Mb/s average payload rate.
The original receiver verdict is **`passed=false`**. It observed 1,824 missing
packets in 30 gaps, maximum individual gap 173 packets. The corresponding
30 sample-index discontinuities amount to 933,888 missing 16-bit words,
not an additional independent loss count. First packet sequence and sample
index were zero. Duplicate, out-of-order, origin, malformed, metadata,
received-payload PRBS16-byte and receive-completion errors were all zero.
The 1-second rate range was 393.900884–398.100834 Mb/s. Unreceived packet
contents were not checked, and must not be described as verified.

The post-STOP UART status shows V9, calibrated MIG, no fatal
state, stopped production, empty ingress and DDR occupancy, and zero listed
ingress/ring/TX overflow or underflow counters. It also reports
`command_rejects=1`; the available status does not identify that command or
its timing. Neither these FPGA counters nor previous host-load observations
load establishes the packet-loss boundary. Host scheduling, NIC/driver,
link/PHY and FPGA-side causes have not been independently excluded.

This is a valid final *measured-loss endurance result*, consistent with the
way earlier Project2 long-run nonzero-loss data was preserved rather than
hidden. It does **not** pass the unchanged strict Gate 5 one-hour zero-loss
criterion in [`board_test_package/VALIDATION_PLAN_V9.md`](board_test_package/VALIDATION_PLAN_V9.md). Project2's planned
engineering validation and evidence collection can close with this known
limitation; “zero-loss 400 Mb/s qualification” or “perfect full-validation
PASS” would be unsupported. Another run is not required solely to archive
and report the result. A truly lossless operating point and a proven loss
boundary remain optional future investigation, not achieved claims.
