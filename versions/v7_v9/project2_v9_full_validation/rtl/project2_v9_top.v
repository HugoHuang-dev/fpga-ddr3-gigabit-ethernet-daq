...
// File    : project2_v9_top.v
// Module  : project2_v9_top
// Created : 2026-09-14
// Revised : 2026-09-23
...
module project2_v9_top (
    ...
    wire prbs_valid,xadc_valid,prbs_finite_done,xadc_finite_done;
    wire [15:0] prbs_data,xadc_data;
    wire [31:0] prbs_run_words,xadc_run_words,xadc_drop_count;
    wire [63:0] prbs_total_words,xadc_total_words,source_total_words;
    wire [3:0] xadc_valid_mask;
    wire [11:0] xadc_temperature_raw,xadc_vccint_raw;
    wire [11:0] xadc_vccaux_raw,xadc_vccbram_raw;
    ...
    wire [31:0] packet_sequence; wire [63:0] packet_first_word_index;
    wire [13:0] tx_fifo_byte_count;
    ...
    reg [7:0] source_select_meta_ui,source_select_ui;
    ...
    always @(posedge ui_clk) begin
        if(adma_reset) begin
            ...
            source_select_meta_ui<=control_source_select;
            source_select_ui<=source_select_meta_ui;
        end
    end

    assign source_valid = control_source_select == 8'd1 ? xadc_valid : prbs_valid;
    assign source_data = control_source_select == 8'd1 ? xadc_data : prbs_data;
    assign control_finite_done = control_source_select == 8'd1 ?
                                 xadc_finite_done : prbs_finite_done;
    assign control_run_words = control_source_select == 8'd1 ?
                               xadc_run_words : prbs_run_words;
    assign source_total_words = control_source_select == 8'd1 ?
                                xadc_total_words : prbs_total_words;

    v9_uart_control #(.CLK_HZ(125000000),.BAUD(115200)) u_uart_control(
        ...
        .xadc_temperature_raw(xadc_temperature_raw),
        .xadc_vccint_raw(xadc_vccint_raw),.xadc_vccaux_raw(xadc_vccaux_raw),
        .xadc_vccbram_raw(xadc_vccbram_raw),
    ...
    v7_rate_controlled_prbs #(.CLK_HZ(125000000),.PRBS_SEED(16'hACE1)) u_prbs_source(
        .clk(clk_125m),.reset(reset),.clear_counters(clear_125),
        .enable(control_run_enable && source_run && control_source_select == 8'd0),
        .ready(source_ready),.valid(prbs_valid),.data(prbs_data),
        .rate_words_per_sec(control_rate_words_per_sec),
        .finite_mode(control_finite_mode),.finite_words(control_finite_words),
        .finite_done(prbs_finite_done),.run_words(prbs_run_words),
        .total_words(prbs_total_words));

    v8_xadc_stream_source #(.CLK_HZ(125000000),.SAMPLE_RATE_HZ(1000)) u_xadc_source(
        .clk(clk_125m),.reset(reset),.clear_counters(clear_125),
        .enable(control_run_enable && source_run && control_source_select == 8'd1),
        .ready(source_ready),.finite_mode(control_finite_mode),
        .finite_words(control_finite_words),.valid(xadc_valid),.data(xadc_data),
        .finite_done(xadc_finite_done),.run_words(xadc_run_words),
        .total_words(xadc_total_words),.drop_count(xadc_drop_count),
        .valid_mask(xadc_valid_mask),.temperature_raw(xadc_temperature_raw),
        .vccint_raw(xadc_vccint_raw),.vccaux_raw(xadc_vccaux_raw),
        .vccbram_raw(xadc_vccbram_raw));
    ...
    ddr3_axi_mig_wrapper u_ddr3(
        ...
        .device_temp_i(xadc_temperature_raw),
    ...
    udp_v9_tx_fifo_packetizer #(.BURST_BYTES(BURST_BYTES),.FIFO_BURSTS(8),
        .POST_READY_IDLE_CYCLES(950)) u_packetizer(
        .clk(ui_clk),.reset(adma_reset),.clear_counters(clear_ui),
        .configured_payload_bytes(packet_length_ui),
        .source_id(source_select_ui),.user_rd_valid(user_rd_valid),
        .user_rd_last(user_rd_last),.user_rd_data(user_rd_data),
        ...
        .first_word_index(packet_first_word_index),
        .framing_error(packetizer_error),
    ...
    // Each ILA observes signals native to its own sample clock. Do not add a
    // foreign-domain bus here; synchronize a status bit first or use the ILA
    // attached to the bus owner's clock domain.
    ila_v9_acq u_ila_acq(
        .clk(clk_125m),
        .probe0(control_run_enable),.probe1(control_source_select),
        .probe2(source_valid),.probe3(source_ready),.probe4(source_data),
        .probe5(ingress_level_words),.probe6(ingress_has_burst),
        .probe7(source_run),.probe8(ingress_high_water),
        .probe9(ingress_low_water),.probe10(ingress_drain_busy),
        .probe11(user_wr_en),.probe12(user_wr_data),
        .probe13(ingress_overflow_count),.probe14(ingress_underflow_count),
        .probe15(xadc_valid_mask),.probe16(xadc_drop_count),
        .probe17(control_finite_done),.probe18(control_run_words));

    ila_v9_ui u_ila_ui(
        .clk(ui_clk),
        .probe0(init_calib_complete),.probe1(ring_started),
        .probe2(drain_active),.probe3(write_inflight),.probe4(read_inflight),
        .probe5(write_pointer),.probe6(read_pointer),.probe7(occupancy_bytes),
        .probe8(committed_bytes[31:0]),.probe9(released_bytes[31:0]),
        .probe10(ring_full),.probe11(ring_empty),
        .probe12(tx_fifo_byte_count),.probe13(tx_fifo_packet_count),
        .probe14(tx_burst_space_available),.probe15(packet_done),
        .probe16(packet_sequence),.probe17(packet_first_word_index),
        .probe18(ring_overflow_count),.probe19(ring_underflow_count),
        .probe20(tx_overflow_count),.probe21(tx_underflow_count),
        .probe22(fatal_error),.probe23(bvalid),.probe24(bready),
        .probe25(arvalid),.probe26(arready),.probe27(rvalid),
        .probe28(rready),.probe29(rlast),.probe30(awvalid),
        .probe31(awready),.probe32(wvalid),.probe33(wready),
        .probe34(tx_burst_committed));

    ila_v9_rx u_ila_rx(
        .clk(phy_rx_clk),.probe0(gmii_rx_data_vld),
        .probe1(gmii_rx_data_error),.probe2(gmii_rx_data));
endmodule
