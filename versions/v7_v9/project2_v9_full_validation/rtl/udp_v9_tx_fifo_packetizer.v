...
// File    : udp_v9_tx_fifo_packetizer.v
// Module  : udp_v9_tx_fifo_packetizer
// Created : 2026-09-14
// Revised : 2026-09-23
...
// V9 keeps DDR transfers fixed at BURST_BYTES but can divide each complete
// burst into 256, 512 or 1024-byte UDP payloads. Ring space is released when
// the complete DDR burst enters this FIFO, exactly as in V8. The 32-byte V9
// header adds a 64-bit word index so the PC can validate every sample without
// retaining the complete raw stream.
module udp_v9_tx_fifo_packetizer #(
    ...
    output reg  [63:0] first_word_index,
    ...
    function [7:0] header_byte;
        ...
        begin
            ...
            case (index)
                ...
                23: header_byte = occupancy[7:0];
                24: header_byte = word_index[63:56];
                25: header_byte = word_index[55:48];
                26: header_byte = word_index[47:40];
                27: header_byte = word_index[39:32];
                28: header_byte = word_index[31:24];
                29: header_byte = word_index[23:16];
                30: header_byte = word_index[15:8];
                31: header_byte = word_index[7:0];
    ...
    always @(posedge clk) begin
        if (reset) begin
            ...
            packet_done<=0; packet_sequence<=0; first_word_index<=0;
            framing_error<=0;
            ...
            if (clear_counters) begin
                ...
                packet_sequence<=0; first_word_index<=0; framing_error<=0;
                overflow_count<=0; underflow_count<=0;
                starvation_latched<=0;
            end else begin
