...
// File    : v8_uart_control.v
// Module  : v8_uart_control
// Created : 2026-09-08
// Revised : 2026-09-22
...
module v8_uart_control #(
    ...
    input  wire [3:0]   xadc_valid_mask,
    input  wire [31:0]  xadc_drop_count,
    input  wire [11:0]  xadc_temperature_raw,
    input  wire [11:0]  xadc_vccint_raw,
    input  wire [11:0]  xadc_vccaux_raw,
    input  wire [11:0]  xadc_vccbram_raw,
    ...
    always @(posedge clk) begin
        if (reset) begin
            ...
            for (ri=0; ri<80; ri=ri+1) response_payload[ri] <= 0;
        end else begin
            uart_tx_en <= 0;
            tx_crc_reset <= 0;
            tx_crc_feed <= 0;
            ...
            if (status_waiting &&
                ...
                response_payload[61] <= {4'd0,xadc_valid_mask};
                response_payload[62] <= xadc_drop_count[31:24];
                response_payload[63] <= xadc_drop_count[23:16];
                response_payload[64] <= xadc_drop_count[15:8];
                response_payload[65] <= xadc_drop_count[7:0];
                response_payload[66] <= {4'd0,xadc_temperature_raw[11:8]};
                response_payload[67] <= xadc_temperature_raw[7:0];
                response_payload[68] <= {4'd0,xadc_vccint_raw[11:8]};
                response_payload[69] <= xadc_vccint_raw[7:0];
                response_payload[70] <= {4'd0,xadc_vccaux_raw[11:8]};
                response_payload[71] <= xadc_vccaux_raw[7:0];
                response_payload[72] <= {4'd0,xadc_vccbram_raw[11:8]};
                response_payload[73] <= xadc_vccbram_raw[7:0];
                response_pending <= 1'b1;
            end
