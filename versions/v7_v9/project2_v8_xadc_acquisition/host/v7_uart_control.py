#!/usr/bin/env python3
"""Build, send and decode Project2 V7 UART control frames."""
...
def read_frame(serial_port, timeout: float) -> bytes:
    ...
    while time.monotonic() < deadline:
        ...
        if state == 0:
            ...
            state = 1 if byte == b"\xA5" else 0
    raise TimeoutError("timed out waiting for a complete V7 reply")
...
def decode_reply(frame: bytes) -> str:
    ...
    if frame_type != 0x95 or len(payload) != 61:
    ...
    with serial.Serial(args.port, args.baud, timeout=0.05) as port:
        ...
        try:
            ...
            if args.expect_no_reply:
                ...
                return 0
            raise SystemExit("timed out waiting for a complete V7 reply")
    if args.expect_no_reply:
