...
// File    : tb_v7_uart_control.v
// Module  : tb_v7_uart_control
// Created : 2026-09-02
// Revised : 2026-09-23
...
module tb_v7_uart_control;
    ...
    v7_uart_control #(.CLK_HZ(1000000),.BAUD(100000)) dut(
    ...
    initial begin
        ...
        send_frame(8'h13,8'h06,5,0); wait_reply(48);
        if(!finite_mode || finite_words!=1024) begin $display("FAIL FINITE"); $finish; end

        send_frame(8'h03,8'h07,0,0); wait_reply(56);
        ...
        send_frame(8'h15,8'h08,0,0); wait_reply(count_before+68);
        if(tx_bytes[count_before+2]!=8'h95 || tx_bytes[count_before+4]!=61 ||
           tx_bytes[count_before+5]!=0 || tx_bytes[count_before+6]!=7 ||
           tx_bytes[count_before+7]!=8'h1C) begin
        ...
        $display("V7 UART CONTROL SIM PASSED: bytes=%0d rejects=%0d crc_errors=%0d",
