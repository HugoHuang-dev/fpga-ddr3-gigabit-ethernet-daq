...
// File    : tb_v8_packetizer.v
// Module  : tb_v8_packetizer
// Created : 2026-09-08
// Revised : 2026-09-23
...
module tb_v8_packetizer;
    ...
    udp_v8_tx_fifo_packetizer #(.BURST_BYTES(1024),.FIFO_BURSTS(2),
        .POST_READY_IDLE_CYCLES(2)) dut(
        .clk(clk),.reset(reset),.clear_counters(clear),
        .configured_payload_bytes(16'd256),.source_id(8'd1),.user_rd_valid(rd_valid),
    ...
    always @(posedge clk) begin
        if(app_valid) begin
            ...
            if(wire_index==3 && app_data!="8") begin $display("FAIL magic 8");$finish;end
            if(wire_index==4 && app_data!=8) begin $display("FAIL version");$finish;end
            if(wire_index==5 && app_data!=1) begin $display("FAIL source id");$finish;end
    ...
    initial begin
        ...
        $display("V8 PACKETIZER SIM PASSED: source-tagged burst -> %0d x 256-byte packets",packets);
        $finish;
    end
endmodule
