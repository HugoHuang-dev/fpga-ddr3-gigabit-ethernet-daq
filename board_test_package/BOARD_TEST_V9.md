...
`--first-timeout` begins when the receiver reports ARMED, not when START is sent. Allow time for manual window switching. Source 1 may also need roughly 33 seconds to accumulate its first 64 KiB DDR output batch. The earlier [Gate 1 JSON](results/v9_gate1_prbs_1024B_25M_60s.json) records a **zero-packet timeout before START**; retain it as a failed attempt, not acceptance evidence. The retest uses `*_manual.json` and `gate1_manual`.
...
The [board-validation record](../versions/v7_v9/project2_v9_full_validation/evidence/board_20260923/V9_BOARD_EVIDENCE_FINAL.md) reports the actual outcome: Gates 1–3 passed. Gate 5 completed the full 3600 seconds, but receiver-detected missing packets failed the strict zero-loss criterion. Each gate is assessed against its own V9 result.
